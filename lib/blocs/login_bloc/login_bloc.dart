...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
    ...
    if (event is DoctorLoginEvent) {
      ...
      try {
        ...
        if (doctorFound.length >= 1) {
          final doctor =
              await apiHelper.getDocotrDataAfterLogin(doctorFound.first.id);
          final singleDoctor = DoctorModel.fromJson(doctor.data()!);
          await saveDoctorData(doctor: singleDoctor);

          DoctorManager.doctor = singleDoctor;
  ...
  Future saveDoctorData({required DoctorModel doctor}) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    final isLogedIn =
    ...
    final sharedPred = await sharedPreferences.setString(
        FirebaseStrings.doctorKey, json.encode(doctor.toJson()));
    print('sharedprefrences $sharedPred  data savedddd');
  }
}
